...
import com.radioroam.android.ui.utility.findActivity
        ...
        val observer = LifecycleEventObserver { _, event ->
            when {
                ...
                (event == Lifecycle.Event.ON_PAUSE && Build.VERSION.SDK_INT <= 23)
                    ...
                    playerManager.release(currentContext.findActivity()?.isChangingConfigurations == true)
                }
            }
        }
...
internal class PlayerManager(
    ...
    internal fun release(isConfigChanging: Boolean = false) {
        if (isConfigChanging)
            return
